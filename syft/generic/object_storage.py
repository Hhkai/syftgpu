...
import torch
gpuopen = True
device = torch.device("cuda:0" if gpuopen and torch.cuda.is_available() else "cpu")
...
class ObjectStore:
    ...
    def set_obj(self, obj: Union[FrameworkTensorType, AbstractTensor]) -> None:
        ...
        # hhk
        id_stored = obj.id
        native_id_stored = obj.native_id
        # cdata_stored = obj._cdata
        if isinstance(obj, torch.Tensor):
            if not (hasattr(obj, 'child') and isinstance(obj.child, syft.AdditiveSharingTensor)):
                obj = obj.to(device)
                obj.id = id_stored
                obj.native_id = native_id_stored
                obj.owner = self.owner
                # obj._cdata = cdata_stored
