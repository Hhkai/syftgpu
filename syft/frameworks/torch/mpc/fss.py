...
def fss_op(x1, x2, op="eq"):
    ...
    try:
        ...
        if sy.local_worker.crypto_store.force_preprocessing:
            raise
        class_attributes['crypto_provider'].crypto_store.provide_primitives(workers=locations, **e.kwargs_)
...
def mask_builder(x1, x2, op):
    ...
    # hhk
    r = x + th.tensor(alpha.astype(np.int64)).reshape(x.shape).to(device)
...
def eq_evaluate(b, x_masked):
    ...
    return th.tensor(result_share).to(device)
...
def comp_evaluate(b, x_masked, owner_id=None, core_id=None, burn_offset=0, dtype=None):
    ...
    result_share = DIF.eval(b.cpu().numpy().item(), x_masked.cpu().detach().numpy(), s_0, *CW)

    dtype_options = {None: th.long, "int": th.int32, "long": th.long, "int24": th.float64 }
    result = th.tensor(result_share, dtype=dtype_options[dtype]).to(device)
    if core_id is None:
        return result
    else:
