...
class FixedPrecisionTensor(AbstractTensor):
    ...
    def fix_precision(self, check_range=True):
        ...
        # hhk
        upscaled = (rational * self.base ** self.precision_fractional).long().double()
        if check_range:
            assert (
    ...
    def float_precision(self):
        ...
        result = (neg_nums + pos_nums).double() / (self.base ** self.precision_fractional)
    ...
    def _tanh_chebyshev(tensor, maxval: int = 6, terms: int = 32):
        ...
        value = torch.tensor(maxval).cuda().fix_precision(**tensor.get_class_attributes())
