...
from syft.generic.utils import int48module
# from syft.generic.utils import int24module
...
class AdditiveSharingTensor(AbstractTensor):
    ...
    def __str__(self):
        ...
        if self.crypto_provider is not None:
            out += f"*crypto provider: {self.crypto_provider.id}*"
    ...
    def modulo(self, x):
        # hhk : must fix
        # hhk debug 11.13
        return int48module(x)
        if hasattr(x, 'dtype') and x.dtype == torch.int32:
            # hhk: todo (for snn-relu)
            return int24module(x)
        if self.dtype == "custom":
            ...
            x = (x + self.field) % self.field
            mask_pos = x > self.max_value
            if mask_pos.any():
                mask_pos = mask_pos.type(self.torch_dtype)
                x = x - (mask_pos * self.field)
            return x.to(device)
        else:
            return x
    ...
    def get(self):
        ...
        result = self.modulo(sum(shares)) # hhk:todo
        # result = self.modulo(sum(shares) * 100) / 100
        return result

    def get100(self):
        """most of this function is copied from 'get'
        to get the correct ans, we mul and div by 100
        """
        ...
        frac = 1000
        result = self.modulo(sum(shares) * frac) / frac
        return result
    ...
    def generate_shares(self, secret, n_workers, random_type):
        ...
        """
        # hhk has commended the line below
        #random_type = torch.LongTensor if random_type == torch.int64 else torch.IntTensor
        if random_type == torch.int64:
            random_type = torch.LongTensor
        elif random_type == torch.float64:
            random_type = torch.cuda.DoubleTensor
        else:
            # hhk 20201118: for the mul in PC , fix the else.
            random_type = torch.LongTensor

        if not isinstance(secret, random_type):
            #print("hhk warning:368 todo")
            # hhk the type should be fixed
        ...
        #random_shares = [torch.tensor([3000.])]   # hhk
        shares = []
    ...
    def _basic_arithmetic_op(self, op, shares: dict, operand):
        ...
        if isinstance(operand, int):
            operand = torch.tensor([operand], dtype=self.torch_dtype).to(device)

        if isinstance(operand, (torch.LongTensor, torch.IntTensor, torch.Tensor, torch.cuda.DoubleTensor, torch.DoubleTensor)):
            # hhk TODO : these types should be reduced
    ...
    def _one_hot_to_index(self, dim, keepdim):
        ...
        if dim is None:
            result = self.flatten()
            n_elem = result.numel()
            result = result * torch.tensor(list(range(n_elem)), dtype=self.torch_dtype, device='cuda:0')
            return result.sum()
        else:
            size = [1] * self.dim()
            size[dim] = self.shape[dim]
            n_elem = self.shape[dim]
            result = self * torch.tensor(list(range(n_elem)), dtype=self.torch_dtype, device='cuda:0').view(size)
